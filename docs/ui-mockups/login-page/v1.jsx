...
import { Shield, Lock, Mail, User, KeyRound, ArrowRight, Fingerprint, AlertTriangle } from 'lucide-react';
...
export default function AuthPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const action = isLogin ? "Connexion" : "Inscription (Vérification EPIS en cours...)";
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-emerald-500/30 relative flex flex-col items-center justify-center p-6 overflow-hidden">
      ...
      <main className="relative z-10 w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <SecurityBadge text="256-bit AES Encrypted" />
        </div>
        ...
        <div className="bg-zinc-900/40 backdrop-blur-2xl border border-white/10 rounded-[2rem] p-8 shadow-[0_20px_60px_rgba(0,0,0,0.5)] relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col">
            ...
            {/* --- CHAMPS COMMUNS --- */}
            <GlassInput icon={Mail} type="email" name="email" placeholder="Secure Email Address" required />
            <GlassInput icon={Lock} type="password" name="password" placeholder="Master Password" required />

            {/* --- CHAMPS SPÉCIFIQUES INSCRIPTION (KYC Belge) --- */}
            {!isLogin && (
              <div className="animate-in fade-in slide-in-from-top-4 duration-300">
                ...
                <div className="my-6 border-t border-white/5 pt-6">
                  <div className="flex items-center gap-2 mb-4 text-rose-400">
                    <Fingerprint size={16} />
                    <span className="text-[10px] font-bold uppercase tracking-widest">Mandatory Identity Check (BE)</span>
                  </div>
                  
                  <GlassInput icon={User} type="text" name="niss" placeholder="Registre National (NISS) ex: 90.01.01-123.45" required />
                  ...
                  <div className="relative group mb-4">
                    ...
                    <input
                      ...
                      name="birthdate"
                      required
                      className="w-full bg-zinc-900/50 border border-white/10 text-white text-sm rounded-xl pl-14 pr-4 py-3.5 focus:outline-none focus:border-emerald-500/50 transition-all [&::-webkit-calendar-picker-indicator]:filter [&::-webkit-calendar-picker-indicator]:invert"
                    />
                  </div>

                  {/* Avertissement Légal */}
                  <div className="bg-rose-950/20 border border-rose-900/30 rounded-lg p-3 flex gap-3 mt-4">
                    <AlertTriangle size={16} className="text-rose-500 flex-shrink-0 mt-0.5" />
                    <p className="text-[10px] text-zinc-400 leading-relaxed">
                      En créant un compte, j'accepte que mon identité soit vérifiée sur la base de données <strong>EPIS</strong> de la Commission des jeux de hasard. Réservé aux personnes de 21 ans et plus.
                    </p>
                  </div>
                </div>
              </div>
            ...
            >
              {isLogin ? (
                ...
                <>Initialize Account <Shield size={16} className="group-hover:scale-110 transition-transform" /></>
              )}
            </button>
